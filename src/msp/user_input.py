...
def parse_command_line_input() -> UserInput:
    ...
    gui.add_argument(
        "-g",
        "--gui",
        help="Run app in a graphical user interface.",
        action="store_true",
    )
...
def get_command_line_arguments(command_line_info: Namespace) -> UserInput:
    ...
    if alignments_position := command_line_info.alignments_position:
        user_input.alignments_position = check_alignments_position(alignments_position)
    else:
    ...
    if annotate_genes := command_line_info.annotate_genes:
        ...
        user_input.annotate_genes_on_sequence = get_annotate_genes_info(annotate_genes)
    else:
...
def check_if_user_provided_only_gui_argument(command_line_info: Namespace) -> None:
...
def check_mandatory_arguments(user_input: Namespace) -> None:
    ...
    if not user_input.input_files and not user_input.gui:
        sys.exit("Error: the `--input` argument is mandatory.")
    if user_input.input_files and user_input.gui:
        sys.exit(
...
def check_figure_name_and_format(user_input: Namespace) -> None:
    ...
    if len(extension) == 1:
        user_input.figure_name = user_input.figure_name + "." + user_input.figure_format
